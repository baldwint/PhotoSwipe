...
// pid - Picture index
// gid - Gallery index
Gc=function(){var a=Ec(),b={};if(i.simpleHashes){if(0==a.length)return b;a.includes("&")||(
// if hash is not an ampersand-delimited set of keys and
// values, interpret it as the pid
b.pid=a)}else{if(a.length<5)// pid=1
...
return void(tc=setTimeout(Hc,500));uc?clearTimeout(sc):uc=!0;var a=m+1,b=$b(m);if(b.hasOwnProperty("pid")&&(
// carry forward any custom pid assigned to the item
a=b.pid),i.simpleHashes)var c=xc+a;else var c=xc+"&gid="+i.galleryUID+"&pid="+a;yc||Bc.hash.indexOf(c)===-1&&(Ac=!0);var d=Bc.href.split("#")[0]+"#"+c;Cc?"#"+c!==window.location.hash&&history[yc?"replaceState":"pushState"]("",document.title,d):yc?Bc.replace(d):Bc.hash=c,yc=!0,sc=setTimeout(function(){uc=!1},60)};ya("History",{publicMethods:{initHistory:function(){if(e.extend(i,Dc,!0),i.history){Bc=window.location,Ac=!1,zc=!1,yc=!1,xc=Ec(),Cc="pushState"in history,d.simpleHashes&&(
// if simple hash is enabled, there's nothing else in
// the hash besides the PID, so initialHash is empty
xc=""),xc.indexOf("gid=")>-1&&(xc=xc.split("&gid=")[0],xc=xc.split("?gid=")[0]),Ba("afterChange",f.updateURL),Ba("unbindEvents",function(){e.unbind(window,"hashchange",f.onHashChange)});var a=function(){wc=!0,zc||(Ac?history.back():xc?Bc.hash=xc:Cc?
